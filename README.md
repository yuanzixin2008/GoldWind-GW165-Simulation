...
The efficiency inputs are based on technical information, but the plotted curves are still generated by **my simplified equation**. They should therefore be understood as **modelled results**, not an official measured power curve.
...
The model uses the **165 m rotor diameter** and **6 MW rated power** of the Goldwind **GW165-6.0MW**. It applies a **cut-in speed of 3 m/s**, limits output at **6,000 kW**, and sets output to zero above the cut-out speed. The displayed line charts focus on **0–15 m/s** so that the comparison around the rise to rated output remains clear. The intermediate curve is calculated from the **simplified equation** rather than copied from the turbine's official measured power curve.
